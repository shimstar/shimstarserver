...
class ShimItem(object):
    ...
    def loadFromBdd(self):
        query = "SELECT star006_template_star004,star006_container_starnnn,star006_containertype,star006_owner_star001,star006_location,star006_nb,star006_enabled "
        ...
        for row in result_set:
            ...
            self.nb = int(row[5])
            self.enabled =True if int(row[6]) == 1 else False
        cursor.close()
    ...
    def saveToBDD(self):
        shimDbConnector.lock.acquire()
        if self.id == 0:
            query = "INSERT INTO STAR006_ITEM (star006_template_star004,star006_container_starnnn,star006_containertype,star006_location,star006_id_star036,star006_nb,star006_enabled) "
            query += " values ('" + str(self.template) + "','" + str(
                self.container) + "','" + self.containertype + "','" + str(self.location) + "','" + str(
                self.mission) + "','" + str(self.nb) + "'"
            if self.enabled:
                query+= ",1)"
            else:
                query+=",0)"
        else:
            query = "UPDATE STAR006_ITEM SET star006_container_starnnn='" + str(
                self.container) + "', star006_containertype='" + self.containertype + "', star006_location='" + str(
                self.location) + "'"
            query += ", star006_id_star036='" + str(self.mission) + "',star006_nb='" + str(self.nb) + "'"
            if self.enabled:
                query+= ",star006_enabled = 1"
            else:
