...
class Ship(ShimItem, threading.Thread):
    className = "ship"
    ...
    def loadFromTemplate(self):
        ...
        query += ", star004_name"
        query += " FROM STAR005_SHIP_TEMPLATE join star004_item_template on star005_id=star004_specific_starxxx AND star004_type_star003 = '" + str(
            C_ITEM_SHIP) + "'"
    ...
    def loadFromBDD(self):
        ...
        cursor = instanceDbConnector.getConnection().cursor()
        query = "SELECT star004_type_star003,star006_id FROM star006_item item JOIN star004_item_template itemTemplate ON item.star006_template_star004 = itemTemplate.star004_id WHERE star006_containertype ='star007_ship' and star006_container_starnnn='" + str(
            self.id) + "'"
        cursor.execute(query)
        result_set = cursor.fetchall()
        for row in result_set:
    ...
    def runPhysics(self):
        ...
        if self.bodyNP is not None and self.bodyNP.isEmpty() == False:
            if self.worldNP is not None:
                ...
                #~ print self.pyr
                v = Vec3(self.pyr['y'] * self.torque * self.slowAngular, 0.0,
                         self.pyr['p'] * self.torque * self.slowAngular)
                ...
                self.bodyNP.node().applyCentralForce(
                    Vec3(forwardVec.getX() * self.poussee, forwardVec.getY() * self.poussee,
                         forwardVec.getZ() * self.poussee))

                self.bodyNP.node().setLinearVelocity((self.bodyNP.node().getLinearVelocity().getX() * self.frictionVelocity,
                                                      self.bodyNP.node().getLinearVelocity().getY() * self.frictionVelocity,
                                                      self.bodyNP.node().getLinearVelocity().getZ() * self.frictionVelocity))
                self.bodyNP.node().setAngularVelocity((
    ...
    def deleteFromBdd(self):
        ...
        query = "DELETE FROM STAR006_ITEM WHERE  star006_containertype='star007_ship' and  star006_container_starnnn='" + str(
            self.id) + "'"
        cursor = instanceDbConnector.getConnection().cursor()
        cursor.execute(query)
        cursor.close()
        ...
        query = "DELETE FROM STAR006_ITEM where star006_id =(SELECT star007_item_star006 from star007_ship where star007_id = '" + str(
            self.id) + "')"
        cursor = instanceDbConnector.getConnection().cursor()
        cursor.execute(query)
        cursor.close()
    ...
    def saveToBDD(self):
        ...
        if self.id > 0:
            ...
            query += " values ('" + str(self.itemId) + "','1','" + str(self.shipTemplate) + "','" + str(
                self.hullpoints) + "')"
            cursor = instanceDbConnector.getConnection().cursor()
            cursor.execute(query)
        ...
        for it in self.itemInInventory:
            if it.getId() > 0:
                query = "UPDATE STAR006_ITEM SET STAR006_location = '" + str(
                    it.getLocation()) + "', star006_containertype='star007_ship', star006_container_starnnn='" + str(
                    self.id) + "'"
                ...
                query = +" values ('" + str(it.getTemplate()) + "','" + str(self.id) + "','star007_ship','" + str(
                    self.owner.id) + "','" + str(it.getLocation()) + "','0','" + str(it.getNb()) + "')"
                cursor = instanceDbConnector.getConnection().cursor()
                cursor.execute(query)
