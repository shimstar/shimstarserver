...
class User(threading.Thread):
    ...
    def loadFromBdd(self):
        ...
        shimDbConnector.getInstance().resetConnection()
        if self.id == 0:
            query = "SELECT star001_passwd,star001_id,star001_name FROM star001_user WHERE star001_name = '" + str(
                self.name) + "'"
        else:
            query = "SELECT star001_passwd,star001_id,star001_name FROM star001_user WHERE star001_id = '" + str(
                self.id) + "'"
        instanceDbConnector = shimDbConnector.getInstance()
        cursor = instanceDbConnector.getConnection().cursor()
        cursor.execute(query)
